const CACHE='thetis-v2.5.10';
const SHELL=['./','./index.html','./styles.css?v=2.5.10','./app.js?v=2.5.10','./manifest.webmanifest','./version.json','./database/THETIS_Database_MASTER.xlsx','./icons/icon-192.png','./icons/icon-512.png'];
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(SHELL)).then(()=>self.skipWaiting())));
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
self.addEventListener('fetch',e=>{
  if(e.request.method!=='GET')return;
  const u=new URL(e.request.url);
  if(u.hostname.endsWith('open-meteo.com')){e.respondWith(fetch(e.request));return;}
